...
func (ms MongoSession) GetLocation(w http.ResponseWriter, r *http.Request, params httprouter.Params) {
	...
	if err := ms.session.DB("assig273").C("locations").FindId(oid).One(&resp); err != nil {
...
func (ms MongoSession) CreateLocation(w http.ResponseWriter, r *http.Request, params httprouter.Params) {
	...
	ms.session.DB("assig273").C("locations").Insert(data)
...
func (ms MongoSession) DeleteLocation(w http.ResponseWriter, r *http.Request, params httprouter.Params) {
	    ...
	    oid := bson.ObjectIdHex(id)
	    if err := ms.session.DB("assig273").C("locations").RemoveId(oid); err != nil {
		    fmt.Print("Inside fail case")
	        w.WriteHeader(404)
	        return
	    }
...
func (ms MongoSession) UpdateLocation (w http.ResponseWriter, r *http.Request, params httprouter.Params) {
	...
	if err := ms.session.DB("assig273").C("locations").FindId(oid).One(&get); err != nil {
		w.WriteHeader(404)
		return
	}

	na := get.Name
	object := ms.session.DB("assig273").C("locations")
...
func (ms MongoSession) CreateTrip(w http.ResponseWriter, r *http.Request, _ httprouter.Params) {
	...
	if err := ms.session.DB("assig273").C("locations").FindId(starting_id).One(&dbResponse); err != nil {
       	w.WriteHeader(404)
        return
    }
	...
	for len(tripInput.Location_ids)>0{
			...
			for _, loc := range tripInput.Location_ids{
			  	...
			  	id := bson.ObjectIdHex(loc)
					var o Response
					if err := ms.session.DB("assig273").C("locations").FindId(id).One(&o); err != nil {
						w.WriteHeader(404)
						return
					}
	...
	var o2 Response
	if err := ms.session.DB("assig273").C("locations").FindId(Last_loc_id).One(&o2); err != nil {
		w.WriteHeader(404)
		return
	}
	...
	var end Response
	if err := ms.session.DB("assig273").C("locations").FindId(ending_id).One(&end); err != nil {
       	w.WriteHeader(404)
        return
    }
	...
	ms.session.DB("assig273").C("Trips").Insert(tripOutput)
...
func (ms MongoSession) GetTrip(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
    ...
    oid := bson.ObjectIdHex(id)
	var tripOutput TripOutput
	if err := ms.session.DB("assig273").C("Trips").FindId(oid).One(&tripOutput); err != nil {
        w.WriteHeader(404)
        return
    }
...
func (ms MongoSession) UpdateTrip(w http.ResponseWriter, r *http.Request, p httprouter.Params) {
    ...
    oid := bson.ObjectIdHex(id)
	if err := ms.session.DB("assig273").C("Trips").FindId(oid).One(&tripUpdate); err != nil {
        w.WriteHeader(404)
        return
    }
    ...
    var trip_not_visited []string
  	...
  	if err := ms.session.DB("assig273").C("Trip_internal_data").FindId(id).One(&internal); err != nil {
    	...
    	ms.session.DB("assig273").C("Trip_internal_data").Insert(internal)
	...
	for i, location := range theStruct.trip_route{
	  	if  (theStruct.trip_visits[location] == 0){
			...
			var o OutputAddress
			if err := ms.session.DB("assig273").C("locations").FindId(nextoid).One(&o); err != nil {
        		w.WriteHeader(404)
        		return
    		}
    		...
    		nlang:= o.Coordinate.Lang
	  		...
	  		if i == 0 {
				...
				var o OutputAddress
				if err := ms.session.DB("assig273").C("locations").FindId(startingoid).One(&o); err != nil {
        			w.WriteHeader(404)
        			return
    			}
				...
				var o OutputAddress
				if err := ms.session.DB("assig273").C("locations").FindId(startingoid2).One(&o); err != nil {
        			w.WriteHeader(404)
        			return
    			}
    			slat := o.Coordinate.Lat
	...
	c := ms.session.DB("assig273").C("Trip_internal_data")
	id2 := bson.M{"_id": id}
...
func getConnection() *mgo.Session {
    conn, err := mgo.Dial("mongodb://sindh:sindh123@ds057244.mongolab.com:57244/assig273")
